...
impl AppState {
    pub(super) fn pull_inputs_from_window(&mut self) {
        ...
        let whisper_cli = window.get_whisper_cli_path().to_string();
        let whisper_model = window.get_whisper_model_path().to_string();
        let whisper_lang = window.get_whisper_language().to_string();
        let changed = {
            let s = self.settings.data_mut();
            let changed = s.download_audio_on_fetch != download_audio
                || s.upload_audio_to_lingq != upload_audio
                || s.audio_dir != audio_dir
                || s.whisper_cli_path != whisper_cli
                || s.whisper_model_path != whisper_model
                || s.whisper_language != whisper_lang;
            if changed {
                s.download_audio_on_fetch = download_audio;
                s.upload_audio_to_lingq = upload_audio;
                s.audio_dir = audio_dir;
                s.whisper_cli_path = whisper_cli;
                s.whisper_model_path = whisper_model;
    ...
    pub(super) fn filtered_browse_articles(&self) -> Vec<&ArticleSummary> {
        self.browse
            ...
            .iter()
            .filter(|article| {
                !self.browse.only_new || !self.browse.saved_urls.contains(&article.url)
            })
            .collect()
    }
    ...
    pub(super) fn filtered_library_articles(&self) -> Vec<&StoredArticleMeta> {
        ...
        // Heading filtering and the Audio-view audio filter remain here.
        let heading = self.library.heading.trim();
        let audio_only = self.current_view == View::Audio;
    ...
    pub(super) fn sync_to_window(&mut self) {
        ...
        window.set_download_audio_on_fetch(s.download_audio_on_fetch);
        window.set_upload_audio_to_lingq(s.upload_audio_to_lingq);
        window.set_audio_dir(s.audio_dir.clone().into());
        window.set_whisper_cli_path(s.whisper_cli_path.clone().into());
        window.set_whisper_model_path(s.whisper_model_path.clone().into());
        window.set_whisper_language(s.whisper_language.clone().into());
        window.set_last_failed_count(self.last_failed_urls.len() as i32);
        ...
        if self.dirty.browse {
            ...
            let browse_rows = filtered_browse
                ...
                .map(|article| BrowseRow {
                    ...
                    has_audio_hint: article.has_audio_hint,
                })
                .collect::<Vec<_>>();
        ...
        if self.dirty.library {
            ...
            let library_rows = filtered_library
                ...
                .map(|article| LibraryRow {
                    ...
                    audio_downloaded: !article.audio_local_path.is_empty(),
                    audio_duration: crate::audio::format_duration(article.audio_duration_seconds)
                        .into(),
                })
                .collect::<Vec<_>>();
        ...
        if self.dirty.preview || self.dirty.library {
            if let Some(article) = &self.library.preview_article {
                ...
                // Prefer a Whisper-generated transcript (much richer text)
                // when one's been recorded; fall back to the page-extracted
                ...
                window.set_preview_body(body.into());
