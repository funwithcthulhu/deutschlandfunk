...
pub(super) fn wire_callbacks(window: &AppWindow, state: &Rc<RefCell<AppState>>) {
    ...
    let state_clone = state.clone();
    window.on_download_article_audio(move |id| {
        state_clone.borrow_mut().download_article_audio(id as i64);
    });

    let state_clone = state.clone();
    window.on_open_audio_dir(move || {
        state_clone.borrow().open_audio_dir();
    });
    ...
    let state_clone = state.clone();
    window.on_reveal_article_audio(move |id| {
        state_clone.borrow().reveal_article_audio(id as i64);
    });

    let state_clone = state.clone();
    window.on_retry_last_failures(move || {
        state_clone.borrow_mut().retry_last_failures();
    });

    let state_clone = state.clone();
    window.on_transcribe_article(move |id| {
        state_clone.borrow_mut().transcribe_article(id as i64);
    });
    ...
    // deutschlandfunk.de search
    let state_clone = state.clone();
    window.on_site_search(move || {
        let mut app = state_clone.borrow_mut();
        app.browse.search_query = app
            .window
            .upgrade()
            .map(|w| w.get_site_search_query().to_string())
            .unwrap_or_default();
        app.run_site_search();
    });
