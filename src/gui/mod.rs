...
enum AppEvent {
    ...
    FetchFinished {
        message: String,
        failed: Vec<String>,
        /// URLs (raw) of items that failed during this batch, for retry.
...
struct AppState {
    ...
    scraper: Arc<DeutschlandfunkClient>,
    ...
    settings_dirty: bool,
    /// URLs that the most recent fetch/save batch failed on. Cleared at the
...
pub fn run() -> Result<(), slint::PlatformError> {
    ...
    let browse_section_index = sections
        ...
        .unwrap_or(0);
    let current_view = match settings.data().last_view.as_str() {
