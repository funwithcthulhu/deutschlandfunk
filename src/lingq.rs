...
impl LingqClient {
    ...
    pub async fn upload_lesson(&self, request: &UploadRequest) -> Result<UploadResponse> {
        ...
        warn_if_audio_dropped(request, &lesson);
        Ok(UploadResponse {
            lesson_id: lesson.id,
            lesson_url: format!(
...
fn warn_if_audio_dropped(request: &UploadRequest, lesson: &LingqLessonResponse) {
    if audio_path_for_upload(request).is_some()
        && lesson.audio.as_deref().map(str::is_empty).unwrap_or(true)
    {
        log::warn!(
