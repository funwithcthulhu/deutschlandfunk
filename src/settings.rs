...
pub struct AppSettings {
    ...
    // Audio
    /// Directory to save downloaded MP3 files. Empty string => default
    /// `<app_data>/audio/`.
    pub audio_dir: String,
    /// Auto-download audio when fetching articles that have audio attached.
    pub download_audio_on_fetch: bool,
    /// Attach the local MP3 (if present) when uploading a lesson to LingQ.
    pub upload_audio_to_lingq: bool,

    // Whisper transcription (optional)
    /// Path to a whisper.cpp `main` (or `whisper-cli`) binary. Empty disables
    /// the transcription feature.
    pub whisper_cli_path: String,
    /// Path to a Whisper model file (e.g. `ggml-large-v3.bin`).
    pub whisper_model_path: String,
    /// ISO-639-1 language hint for Whisper (e.g. "de" for German).
...
impl Default for AppSettings {
    fn default() -> Self {
        Self {
            ...
            browse_section: "nachrichten".to_owned(),
            ...
            download_audio_on_fetch: true,
            upload_audio_to_lingq: true,
            whisper_cli_path: String::new(),
            whisper_model_path: String::new(),
            whisper_language: "de".to_owned(),
        }
    }
}
...
impl SettingsStore {
    ...
    pub fn in_memory_default() -> Self {
        Self {
            path: std::env::temp_dir().join("deutschlandfunk_reader_settings.json"),
...
mod tests {
    ...
    fn settings_round_trip() {
        let dir = std::env::temp_dir().join("dlf_test_settings");
    ...
    fn settings_malformed_json_uses_defaults() {
        let dir = std::env::temp_dir().join("dlf_test_malformed");
        let _ = std::fs::create_dir_all(&dir);
        ...
        let d = store.data();
        assert_eq!(d.browse_section, "nachrichten");
    ...
    fn settings_missing_file_uses_defaults() {
        let path = PathBuf::from("/tmp/dlf_nonexistent_12345.json");
        let store = SettingsStore::load(path).unwrap();
        assert_eq!(store.data().browse_section, "nachrichten");
    }

    #[test]
    fn settings_partial_json_fills_defaults() {
        let dir = std::env::temp_dir().join("dlf_test_partial");
        let _ = std::fs::create_dir_all(&dir);
